...
class KinesisWriter {
     ...
     * 간편 설정
     * */
    suspend fun putRecords(inputs: List<KinesisWriteData>) {
        val entries = inputs.map { input ->
            PutRecordsRequestEntry {
                partitionKey = input.partitionKey
                data = gson.toJson(input.data.delegate)!!.toByteArray()
            }
        }
        putRecordsRequest(entries)
    }
     ...
     * @param entries 입력할 데이터 객체 목록
     * @return 최종 PutRecordsResponse 객체
     */
    suspend fun putRecordsRequest(entries: List<PutRecordsRequestEntry>) {
