 ...
 * */
class CognitoSessionComponent(private val cognitoId: String, private val cognitoIdClientId: String) {

    /** AwsClient 지연 주입 */
    var aws: AwsClient by LazyAwsClientProperty()

    /**
     * USER_PASSWORD_AUTH 로 로그인 수행
     * @param loginIdentifier  이메일(email)이나 전화번호(phone_number) 도 가능함
     * */
    suspend fun initiateAuth(loginIdentifier: String, password: String): InitiateAuthResponse = aws.cognito.initiateAuth(
        InitiateAuthRequest {
     ...
     *  */
    suspend fun refreshTokens(refreshToken: String): InitiateAuthResponse = aws.cognito.initiateAuth(
        InitiateAuthRequest {
            this.clientId = cognitoIdClientId
