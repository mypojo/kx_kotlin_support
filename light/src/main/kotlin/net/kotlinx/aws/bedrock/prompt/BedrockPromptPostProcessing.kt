 ...
 */
class BedrockPromptPostProcessing {

    @Kdsl
    constructor(block: BedrockPromptPostProcessing.() -> Unit = {}) {
        apply(block)
    }
...
Guidelines:
- Always output plain text only inside the <final_response></final_response> XML tags.
- Do not add any JSON, Markdown, or code formatting.
- Do not add any natural language explanations or comments outside the <final_response> tags.
- Ensure the text inside the tags is complete, readable, and self-contained.
...
""".trimIndent()
     ...
     * - 필요 시 변경 가능
     */
    var userInstruction: String = "Please output only your final plain text response within <final_response></final_response> tags"
