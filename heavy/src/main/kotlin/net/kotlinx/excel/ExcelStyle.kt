 ...
 * 미리 정의된 스타일 세트
 */
class ExcelStyle(excel: Excel, val excelFont: ExcelFont) {
    ...
    val green = XlsStyleSet(
        create {
            fillForegroundColor = IndexedColors.BRIGHT_GREEN1.index //형광색 그린
            fillPattern = FillPatternType.SOLID_FOREGROUND
            alignment = HorizontalAlignment.LEFT
        },
        create {
    ...
    val redRight: CellStyle = create {
        setFont(excelFont.red)
    }

    val blueRight: CellStyle = create {
        setFont(excelFont.blue)
    }
