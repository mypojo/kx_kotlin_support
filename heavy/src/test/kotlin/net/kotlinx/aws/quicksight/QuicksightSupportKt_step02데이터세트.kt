...
class QuicksightSupportKt_step02데이터세트 : BeSpecHeavy() {
    ...
    init {
        ...
        Given("최초 생성된 데모 삭제하기") {
            ...
            Then("데이터세트 생성 by sql") {
                ...
                val sqlQueryMetadatas = athenaModule49.executeAndgetQueryResults("$sqlQuery\nlimit 1", 1).resultSet!!.resultSetMetadata!!.columnInfo!!

                val suff = "dev"
                val setConfig = QuicksightDataSetCreation {
                    this.dataSourceId = "athena-dev"
                    this.dataSetId = "nlRpt-${UUID.randomUUID()}"
                    this.dataSetName = "전체미디어종합-${suff}"
                ...
                aws49.quicksight.createDataSet(setConfig)
            }
