 ...
 * DDB용 간단 모듈
 * */
@Deprecated("enhanced가 나오기 전 임시 사용")
class DynamoDbBasicModule<T : DynamoDbBasic>(
    ...
    suspend fun getItem(data: DynamoDbBasic): T? {
        val item: Map<String, AttributeValue> = dynamo.getItem {
            this.tableName = this@DynamoDbBasicModule.tableName
     ...
     * */
    suspend fun query(data: Map<String, S>, query: String): List<T> {
        val queryResult = dynamo.queryPaginated {
            this.tableName = this@DynamoDbBasicModule.tableName
            this.consistentRead = false
            this.expressionAttributeValues = data
